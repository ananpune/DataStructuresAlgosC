...
int links = 0; //basically size
struct student * link; //this will be our linked list pointer
struct student * current; //this points to the current linked list item, will be used later 
...
void addLink(int id)
{
    ...
    else
    {
        ...
        current = link;
		//check if the next next is null to see where the linked list ends, if the item after the next is not NULL, add the item, we will clear the rest of the list after
        while (current->next->next != NULL)
        {
            current = current->next;
        }
		//freeing the next links space in memory (good practice?)
        free(current->next);
